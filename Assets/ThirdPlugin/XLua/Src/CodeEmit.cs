    ...
    public class CodeEmit
    {
        ...
        private void emitLiteralLoad(ILGenerator il, Type type, object obj, int localIndex)
        {
            ...
            else if (type == typeof(decimal))
            {
                ...
                //Debug.Log(string.Format("{0}.{1}.{2}.{3}--{4}", buffer[0], buffer[1], buffer[2], buffer[3], obj));
        ...
        void emitUpdateIfNeeded(ILGenerator il, LocalBuilder L, LocalBuilder translator, Type type, int luaIndex, int localIndex)
        {
            ...
            {
                //Debug.LogWarning("-----------------emit update:" + type);
                il.Emit(OpCodes.Ldloc, translator);
                il.Emit(OpCodes.Ldloc, L);
        ...
        //private MethodInfo UnityEngine_Debug_Log = typeof(Debug).GetMethod("Log", new Type[] { typeof(object)});
        ...
        MethodBuilder emitMethodWrap(TypeBuilder typeBuilder, List<MethodBase> methodsToCall, bool isIndexer, Type declaringType, string methodDesciption = null)
        {
            ...
            for (int i = 0; i < methodsToCall.Count; i++)
            {
                ...
                for (int j = 0; j < paramInfos.Length; j++)
                {
                    ...
                    {
                        //Debug.Log(paramInfo.Name + "," + paramRawType + "," + paramInfo.DefaultValue);
                        emitLiteralLoad(il, paramRawType, paramInfo.DefaultValue, argStore.LocalIndex);
                        il.Emit(OpCodes.Stloc, argStore);
                    }
                    //Debug.LogWarning(declaringType.Name + "." + method.Name + "." + paramInfos[j].Name + " pos(d):" + argStore.LocalIndex + ", pt:" + paramRawType + ", j:" + j);
                    if (argStoreStart == -1)
                    {
                ...
                for (int j = 0; j < paramInfos.Length; j++)
                {
                    //Debug.LogWarning(declaringType.Name + "." + method.Name + "." + paramInfos[j].Name +" pos:" + (argStoreStart + j) + ", op:" + (paramInfos[j].ParameterType.IsByRef ? OpCodes.Ldloca : OpCodes.Ldloc) + ", j:" + j );
