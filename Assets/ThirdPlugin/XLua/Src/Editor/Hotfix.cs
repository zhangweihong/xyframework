...
namespace XLua
{
    public static class Hotfix
    {
        ...
        public static void HotfixInject()
        {
            ...
            {
                Debug.LogError("You can't inject before the compilation is done");
                return;
            }
			...
			if(!File.Exists(mono_path))
			{
				Debug.LogError(mono_path + " can not find mono!");
			}
...
#endif
            var inject_tool_path = "./XluaTools/Tools/XLuaHotfixInject.exe";
            if (!File.Exists(inject_tool_path))
            {
                Debug.LogError("please install the Tools");
                return;
            }
            ...
            foreach (var injectAssemblyPath in injectAssemblyPaths)
            {
                ...
                Debug.Log(Regex.Replace(hotfix_injection.StandardOutput.ReadToEnd(), @"\s*WARNING: The runtime version supported by this application is unavailable(\s|.)*$", ""));
